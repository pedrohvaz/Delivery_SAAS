...
interface RegisterFormProps {
  ...
  loginHref: string // base (o redirect é anexado)
}

export function RegisterForm({ defaultRedirect, loginHref }: RegisterFormProps) {
      ...
      <div className="mx-auto max-w-xl px-4 py-6">
        <div className="bg-white rounded-2xl border p-6 space-y-5">
          ...
          <div className="text-center text-sm text-muted-foreground">
            Já tem conta?{' '}
            <Link href={`${loginHref}?redirect=${encodeURIComponent(redirect)}`} className="text-primary font-medium hover:underline">
              Entrar
            </Link>
          </div>
        </div>
