...
import { ArrowLeft, Phone, ShoppingBag, MapPin, ChevronRight, LogIn, UserPlus, Trash2, Plus, Star } from 'lucide-react'
...
export default function MinhaContaPage() {
  ...
  // Fallback legado: consulta por telefone (mantido por compatibilidade)
  ...
  const { data: legacyOrders } = useQuery({
    ...
    queryFn: async () => {
      const clean = phone.replace(/\D/g, '')
      const r = await api.get<{ data: AccountOrder[] }>(`/store/${slug}/customer-orders?phone=${clean}`)
      return r.data.data
    },
